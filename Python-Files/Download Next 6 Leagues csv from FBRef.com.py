...
def getRawDataUrl(lg_str,lg_id,season,pageDetail):
    if season == 'current':
        return "https://fbref.com/en/comps/%i/%s/%s-Stats" % (lg_id, pageDetail, lg_str)
    else:
        return "https://fbref.com/en/comps/%i/%s/%s/%s-%s-Stats" % (lg_id, season, pageDetail, season, lg_str)

def getPlayersDataframeFormatted(dataType):
    df = get_players_df(dataType)
...
def joinAllDataframes(df_standard, df_shooting, df_passing, df_pass_types, df_gsca,
                      df_defense, df_poss, df_misc):
    ...
    ## Note that I include all columns but don't always change the names... this is useful to me when I need to update the columns, like when FBRef switched to Opta data haha. I got lucky as this made it easier on me!
    ...
    df = df.rename(
        columns={'Gls': 'Goals', 'Sh': 'Shots', 'SoT': 'SoT', 'SoT%': 'SoT%', 'Sh/90': 'Sh/90', 'SoT/90': 'SoT/90',
                 'G/Sh': 'G/Sh', 'G/SoT': 'G/SoT', 'Dist': 'AvgShotDistance', 'FK': 'FKShots', 'PK': 'PK',
                 'PKatt': 'PKsAtt', 'xG': 'xG', 'npxG': 'npxG', 'npxG/Sh': 'npxG/Sh', 'G-xG': 'G-xG',
                 'np:G-xG': 'npG-xG'})

    df = df.join(df_passing.iloc[:, 8:13])
    df = df.rename(
        columns={'Cmp': 'PassesCompleted', 'Att': 'PassesAttempted', 'Cmp%': 'TotCmp%', 'TotDist': 'TotalPassDist',
    ...
    df = df.rename(
        columns={'Ast': 'Assists', 'xAG': 'xAG', 'xA': 'xA', 'A-xAG': 'A-xAG', 'KP': 'KeyPasses', '1/3': 'Final1/3Cmp',
                 'PPA': 'PenAreaCmp', 'CrsPA': 'CrsPenAreaCmp', 'PrgP': 'ProgPasses', })

    df = df.join(df_pass_types.iloc[:, 9:23])
    df = df.rename(
        columns={'Live': 'LivePass', 'Dead': 'DeadPass', 'FK': 'FKPasses', 'TB': 'ThruBalls', 'Sw': 'Switches',
                 'Crs': 'Crs', 'CK': 'CK', 'In': 'InSwingCK', 'Out': 'OutSwingCK', 'Str': 'StrCK', 'TI': 'ThrowIn',
                 'Off': 'PassesToOff', 'Blocks': 'PassesBlocked', 'Cmp': 'Cmpxxx'})

    df = df.join(df_gsca.iloc[:, 8:16].rename(
        columns={'SCA': 'SCA', 'SCA90': 'SCA90', 'PassLive': 'SCAPassLive', 'PassDead': 'SCAPassDead', 'TO': 'SCADrib',
                 'Sh': 'SCASh', 'Fld': 'SCAFld', 'Def': 'SCADef'}))
    df = df.join(df_gsca.iloc[:, 16:24].rename(
        columns={'GCA': 'GCA', 'GCA90': 'GCA90', 'PassLive': 'GCAPassLive', 'PassDead': 'GCAPassDead', 'TO': 'GCADrib',
                 'Sh': 'GCASh', 'Fld': 'GCAFld', 'Def': 'GCADef'}))

    df = df.join(df_defense.iloc[:, 8:13].rename(
        columns={'Tkl': 'Tkl', 'TklW': 'TklWinPoss', 'Def 3rd': 'Def3rdTkl', 'Mid 3rd': 'Mid3rdTkl',
                 'Att 3rd': 'Att3rdTkl'}))
    df = df.join(df_defense.iloc[:, 13:24].rename(
        columns={'Tkl': 'DrbTkl', 'Att': 'DrbPastAtt', 'Tkl%': 'DrbTkl%', 'Lost': 'DrbPast', 'Blocks': 'Blocks',
                 'Sh': 'ShBlocks', 'Pass': 'PassBlocks', 'Int': 'Int', 'Tkl+Int': 'Tkl+Int', 'Clr': 'Clr',
                 'Err': 'Err'}))

    df = df.join(df_poss.iloc[:, 8:30])
    df = df.rename(
        columns={'Touches': 'Touches', 'Def Pen': 'DefPenTouch', 'Def 3rd': 'Def3rdTouch', 'Mid 3rd': 'Mid3rdTouch',
                 'Att 3rd': 'Att3rdTouch', 'Att Pen': 'AttPenTouch', 'Live': 'LiveTouch', 'Succ': 'SuccDrb',
                 'Att': 'AttDrb', 'Succ%': 'DrbSucc%', 'Tkld': 'TimesTackled', 'Tkld%': 'TimesTackled%',
                 'Carries': 'Carries', 'TotDist': 'TotalCarryDistance', 'PrgDist': 'ProgCarryDistance',
                 'PrgC': 'ProgCarries', '1/3': 'CarriesToFinalThird', 'CPA': 'CarriesToPenArea', 'Mis': 'CarryMistakes',
                 'Dis': 'Disposesed', 'Rec': 'ReceivedPass', 'PrgR': 'ProgPassesRec'})

    df = df.join(df_misc.iloc[:, 8:14])
    df = df.rename(
        columns={'CrdY': 'Yellows', 'CrdR': 'Reds', '2CrdY': 'Yellow2', 'Fls': 'Fls', 'Fld': 'Fld', 'Off': 'Off', })
    df = df.join(df_misc.iloc[:, 17:24])
    df = df.rename(columns={'PKwon': 'PKwon', 'PKcon': 'PKcon', 'OG': 'OG', 'Recov': 'Recov', 'Won': 'AerialWins',
                            'Lost': 'AerialLoss', 'Won%': 'AerialWin%', })

    return df

def individualLeagueData(lg_str, lg_id, season):

    standard = getRawDataUrl(lg_str,lg_id,season,'stats')
    shooting = getRawDataUrl(lg_str,lg_id,season,'shooting')
    passing = getRawDataUrl(lg_str, lg_id, season, 'passing/players')
    pass_types = getRawDataUrl(lg_str, lg_id, season, 'passing_types/players')
    gsca = getRawDataUrl(lg_str, lg_id, season, 'gca/players')
    defense = getRawDataUrl(lg_str, lg_id, season, 'defense/players')
    poss = getRawDataUrl(lg_str, lg_id, season, 'possession/players')
    misc = getRawDataUrl(lg_str, lg_id, season, 'misc/players')

    df_standard = getPlayersDataframeFormatted(standard)
    df_shooting = getPlayersDataframeFormatted(shooting)
    df_passing = getPlayersDataframeFormatted(passing)
    df_pass_types = getPlayersDataframeFormatted(pass_types)
    df_gsca = getPlayersDataframeFormatted(gsca)
    df_defense = getPlayersDataframeFormatted(defense)
    df_poss = getPlayersDataframeFormatted(poss)
    df_misc = getPlayersDataframeFormatted(misc)

    df = joinAllDataframes(df_standard, df_shooting, df_passing, df_pass_types, df_gsca,
                      df_defense, df_poss, df_misc)
    ...
    return df
def getFinalFileForOutputData(root,raw_nongk, final_nongk):
    ##################################################################################
...
df = individualLeagueData('Serie-A', 24, '2022')

raw_nongk = 'Raw FBRef 2022-2023 - %s' %comp
df.to_csv("%s%s.csv" % (root, raw_nongk), index=False)

final_nongk = 'Final FBRef 2022-2023 - %s' %comp
getFinalFileForOutputData(root,raw_nongk, final_nongk)
# The last argument is which season's data you want to extract. Send the season you want data for, and if current season, send 'current'
...
# # %%
# """
# # Run this next line if you want to combine these 6 leagues with the T5 leagues
# """
#
# # %%
# df_t5 = pd.read_csv('%sFinal FBRef 2022-2023.csv' %root)
# df_n6 = pd.read_csv('%sSecond 6 Leagues.csv' %root)
# df = df_t5.append(df_n6)
# df.to_csv('%sFinal FBRef 2022-2023.csv' %root)
#
# df_t5 = pd.read_csv('%sFinal FBRef GK 2022-2023.csv' %root)
# df_n6 = pd.read_csv('%sSecond 6 Leagues GK.csv' %root)
# df = df_t5.append(df_n6)
# df.to_csv('%sFinal FBRef GK 2022-2023.csv' %root)
#
#
# # %%
# """
